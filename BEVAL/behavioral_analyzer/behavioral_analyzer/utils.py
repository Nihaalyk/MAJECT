...
class DataCollector:
    ...
    def __init__(self, session_name: str, start_time: Optional[float] = None, max_data_points: int = 10000):
        self.session_name = session_name
        self.start_time = start_time or time.time()
        self.max_data_points = max_data_points  # Limit memory usage
    ...
    def add_video_data(self, data_point: Dict[str, Any]) -> None:
        """Add video analysis data point (optimized with memory limit)."""
        video_list = self.data['analysis_data']['video_data']
        video_list.append(data_point)
        self.data['statistics']['total_frames'] += 1
        
        # Limit memory: keep only recent data points
        if len(video_list) > self.max_data_points:
            # Keep last max_data_points entries
            self.data['analysis_data']['video_data'] = video_list[-self.max_data_points:]
    
    def add_audio_data(self, data_point: Dict[str, Any]) -> None:
        """Add audio analysis data point (optimized with memory limit)."""
        audio_list = self.data['analysis_data']['audio_data']
        audio_list.append(data_point)
        self.data['statistics']['total_audio_chunks'] += 1
        
        # Limit memory: keep only recent data points
        if len(audio_list) > self.max_data_points:
            self.data['analysis_data']['audio_data'] = audio_list[-self.max_data_points:]
    
    def add_combined_data(self, data_point: Dict[str, Any]) -> None:
        """Add combined analysis data point (optimized with memory limit)."""
        combined_list = self.data['analysis_data']['combined_data']
        combined_list.append(data_point)
        
        # Limit memory: keep only recent data points
        if len(combined_list) > self.max_data_points:
            self.data['analysis_data']['combined_data'] = combined_list[-self.max_data_points:]
