...
class VideoAnalyzer:
    ...
    def analyze_frame(self, frame: np.ndarray) -> np.ndarray:
        """Analyze a single frame and return annotated frame (optimized)."""
        process_start = time.time()
        
        # Optimize: Only copy frame if we need to draw on it
        needs_drawing = (self.config.show_landmarks or 
                        self.config.show_object_detections or
                        hasattr(self, '_always_draw') and self._always_draw)
        
        if needs_drawing:
            display_frame = frame.copy()
        else:
            display_frame = frame  # Use same frame reference
        
        # Convert to RGB for MediaPipe (only if needed)
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        
        # Process with face mesh and pose (optimized: process in parallel if possible)
        # Note: MediaPipe processes are already optimized internally
        ...
        # Perform object detection (optimized: skip if not needed frequently)
        if hasattr(self, 'object_detector') and self.object_detector:
            # Throttle object detection for performance (every 3rd frame)
            if not hasattr(self, '_obj_detection_counter'):
                self._obj_detection_counter = 0
            
            self._obj_detection_counter += 1
            if self._obj_detection_counter >= 3 or not hasattr(self, 'current_detections'):
                ...
                detections = getattr(self, 'current_detections', [])
            
            # Draw object detections if enabled
            if self.config.show_object_detections and needs_drawing:
