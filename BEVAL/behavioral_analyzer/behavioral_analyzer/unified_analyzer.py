...
class UnifiedBehavioralAnalyzer:
    ...
    def __init__(self, config: Config):
        ...
        self._cached_video_state = {}
        self._cached_audio_state = {}
        self._state_update_counter = 0
        
        # Frame skipping for performance
        self._frame_skip_counter = 0
        self._frame_skip_interval = 0  # Process every frame by default
    ...
    def process_frame(self) -> Tuple[bool, Optional[np.ndarray]]:
        ...
        try:
            ...
            # Mirror frame horizontally for natural webcam view (in-place operation)
            frame = cv2.flip(frame, 1)
            
            # Process with video analyzer (optimized to avoid unnecessary copies)
            annotated_frame = self.video_analyzer.analyze_frame(frame)
            
            # Update unified state only if enough time has passed (throttle updates)
            current_time = time.time()
            if not hasattr(self, '_last_state_update') or (current_time - self._last_state_update) >= 0.1:
                self._update_unified_state()
                self._last_state_update = current_time
            
            # Add unified analysis overlay (only if needed)
            if hasattr(self, '_show_overlay') and self._show_overlay:
            ...
            if not hasattr(self, '_last_perf_update') or (current_time - self._last_perf_update) >= 1.0:
                process_time = current_time - self.session_start_time
    ...
    def _update_unified_state(self):
        """Update unified state from individual analyzers (optimized with caching)."""
        # Get video analyzer state (cache to avoid repeated attribute access)
        video_state = {
        ...
        # Get audio analyzer state (cache to avoid repeated calls)
        audio_state = self.audio_analyzer.get_current_state()
        
        # Only update if state actually changed (optimization)
        video_changed = video_state != self._cached_video_state
        audio_changed = audio_state != self._cached_audio_state
        
        if video_changed or audio_changed:
            # Combine states with priority logic
            new_emotion = self._combine_emotions(video_state['emotion'], audio_state['emotion'])
            new_fatigue = self._combine_fatigue(video_state['fatigue'], audio_state)
            
            # Only update if values changed
            if (new_emotion != self.unified_state['emotion'] or
                video_state['attention'] != self.unified_state['attention'] or
                video_state['posture'] != self.unified_state['posture'] or
                video_state['movement'] != self.unified_state['movement'] or
                new_fatigue != self.unified_state['fatigue'] or
                audio_state['transcription'] != self.unified_state['transcription'] or
                abs(audio_state['sentiment'] - self.unified_state['sentiment']) > 0.01 or
                audio_state['confidence'] != self.unified_state['confidence']):
    ...
    def _collect_combined_data(self):
        """Collect combined analysis data (optimized)."""
        current_time = time.time()
        
        # Use cached values to avoid repeated attribute access
        video_metrics = {
            'emotion': self._cached_video_state.get('emotion', self.video_analyzer.current_emotion),
            'attention': self._cached_video_state.get('attention', self.video_analyzer.attention_state),
            'posture': self._cached_video_state.get('posture', self.video_analyzer.posture_state),
            'movement': self._cached_video_state.get('movement', self.video_analyzer.movement_level),
            'fatigue': self._cached_video_state.get('fatigue', self.video_analyzer.fatigue_level),
            'blink_rate': self.video_analyzer.blink_count / max(1, current_time - self.video_analyzer.last_reset_time) * 60.0,
        ...
        combined_data = {
            ...
            'audio_metrics': self._cached_audio_state.copy() if self._cached_audio_state else self.audio_analyzer.get_current_state(),
