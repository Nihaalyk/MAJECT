...
import { RateCalculatorAgent } from "./rate-calculator-agent/RateCalculatorAgent";
...
export class AgentRegistry {
  ...
  private rateCalculatorAgent: RateCalculatorAgent;
  ...
  constructor(memory: ConversationMemory, language: 'en' | 'ms', getContextualInfo?: () => Record<string, unknown>) {
    ...
    this.rateCalculatorAgent = new RateCalculatorAgent({ language, memory });
  }
  ...
  async processToolCall(toolCall: any, context: AgentContext): Promise<AgentResult | null> {
    ...
    try {
      ...
      switch (name) {
        ...
        case "calculate_postage_rate":
          result = await this.rateCalculatorAgent.processRateCalculation(args);
          break;
  ...
  private updateMemoryWithInteraction(functionName: string, args: any, result: any, context: AgentContext): void {
    // Update pending information based on the interaction
    const pendingInfo: any = { ...this.memory.currentContext.pendingInfo };
    
    if (functionName === "calculate_postage_rate") {
      if (args.origin) pendingInfo.origin = args.origin;
      if (args.destination) pendingInfo.destination = args.destination;
      if (args.weight) pendingInfo.weight = args.weight;
      if (args.service_type) pendingInfo.serviceType = args.service_type;
    }

    // Update user preferences if new information is provided
    const userPreferences = { ...this.memory.userPreferences };
    if (args.origin) userPreferences.lastUsedOrigin = args.origin;
    if (args.destination) userPreferences.lastUsedDestination = args.destination;
    if (args.service_type) userPreferences.defaultService = args.service_type;

    // Update memory
    this.memory.currentContext.pendingInfo = pendingInfo;
    this.memory.userPreferences = userPreferences;
  }
  ...
  /**
   * Get service options from main agent
   */
  ...
  updateLanguage(language: 'en' | 'ms', getContextualInfo?: () => Record<string, unknown>): void {
    if (this.currentLanguage !== language) {
      ...
      this.rateCalculatorAgent = new RateCalculatorAgent({ language, memory: this.memory });
    }
  }
