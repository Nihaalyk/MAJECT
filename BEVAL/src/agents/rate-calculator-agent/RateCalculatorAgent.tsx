...
import { calculatePostageRate, RateCalculationParams } from "../../lib/pos-malaysia-services";
...
export class RateCalculatorAgent {
  ...
  private enhanceParamsWithMemory(params: any): any {
    ...
    return {
      ...params,
      origin: params.origin || pendingInfo.origin || userPreferences.lastUsedOrigin,
      destination: params.destination || pendingInfo.destination || userPreferences.lastUsedDestination,
      weight: params.weight || pendingInfo.weight,
      service_type: params.service_type || pendingInfo.serviceType || userPreferences.defaultService,
  ...
  private formatRateMessage(params: any, rateResult: any, language: 'en' | 'ms'): string {
    if (language === 'en') {
      return `The postage rate for ${params.service_type || 'Pos Laju'} from ${params.origin} to ${params.destination} (${params.weight}) is RM ${rateResult.rate}. Estimated delivery time: ${rateResult.estimated_delivery}.`;
    } else {
      return `Kadar pos untuk ${params.service_type || 'Pos Laju'} dari ${params.origin} ke ${params.destination} (${params.weight}) adalah RM ${rateResult.rate}. Anggaran masa penghantaran: ${rateResult.estimated_delivery}.`;
    }
  }
  ...
  getDescription(): string {
    ...
    if (language === 'en') {
      return "Calculates postage rates for POS Malaysia services";
    } else {
      return "Mengira kadar pos untuk perkhidmatan POS Malaysia";
    }
  }
}
