...
export class AudioStreamer {
  ...
  /**
   * Immediately interrupt audio playback for VAD (Voice Activity Detection)
   * This stops audio more aggressively than the regular stop method
   */
  interrupt() {
    this.isPlaying = false;
    this.isStreamComplete = true;
    ...
    // Immediately mute the audio without fade-out for faster interruption
    this.gainNode.gain.setValueAtTime(0, this.context.currentTime);

    // Cancel any scheduled audio sources
    if (this.endOfQueueAudioSource) {
      try {
